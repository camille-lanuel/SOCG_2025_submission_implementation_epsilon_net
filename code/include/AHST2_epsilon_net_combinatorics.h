...
public:
        ...
        std::vector<Anchor> insert(const Point& query, Anchor& anch, std::vector<int>& count_locate);
...
template<class Traits>
std::vector<int> Anchored_hyperbolic_surface_triangulation_2<Traits>::epsilon_net(const double epsilon)
{       
        int insertions = 0;
        std::vector<int> count_locate;
        ...
        int count_zero_locate = 0;
        int max_locate = 0;
        for (int i : count_locate) {
             if (i == 0) {
                     count_zero_locate++;
             }
             if (i > max_locate) {
        ...
        std::vector<int> res = {insertions, total_locate, max_locate, count_zero_locate, total_filps, max_flips};
        return res;
}

...
DemoWindowItem::DemoWindowItem() : CGAL::Qt::GraphicsItem(){
  // Clear
  _edges.clear();
  _dirichlet_edges.clear();

  // Prepare the pens
  _poincare_disk_pen.setStyle(Qt::SolidLine);
  _poincare_disk_pen.setWidth(4);
  _poincare_disk_pen.setBrush(Qt::black);

  _edges_pen.setStyle(Qt::SolidLine);
  _edges_pen.setWidth(4);
  _edges_pen.setBrush(Qt::blue);

  _dirichlet_pen.setStyle(Qt::SolidLine);
  _dirichlet_pen.setWidth(4);
  _dirichlet_pen.setBrush(Qt::red);
}
...
void DemoWindowItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget){
  ...
  painter->setBrush(QBrush());
  painter->setPen(_dirichlet_pen);
  for (int i=0; i<_dirichlet_edges.size(); i++){
	draw_edge(painter, _dirichlet_edges[i].first, _dirichlet_edges[i].second);
  }
}
...
void DemoWindowItem::draw_unfold(Triangulation& triangulation){
  ...
  realized_triangles = unfold(triangulation);
...
void DemoWindowItem::draw_dirichlet(Domain& domain){
  std::vector<CGAL::Circular_arc_point_2<Kernel>> vertices = compute_dirichlet_vertices(domain);
  int n = vertices.size();
  for (int i=0; i<n; ++i){
  	Point v0 = Point(CGAL::to_double(vertices[i].x()), CGAL::to_double(vertices[i].y()));
  	Point v1 = Point(CGAL::to_double(vertices[(i+1)%n].x()), CGAL::to_double(vertices[(i+1)%n].y()));
  	_dirichlet_edges.push_back(std::make_pair(v0, v1));
  }
}
...
void DemoWindowItem::draw_triangles(std::vector<Anchor> anchors){
  for (int i=0; i<anchors.size(); i++){
	...
	_edges.push_back(std::make_pair(p0, p1));
	_edges.push_back(std::make_pair(p1, p2));
	_edges.push_back(std::make_pair(p2, p0));
  }
}

void DemoWindowItem::draw_triangulation(Anchored_Triangulation& triangulation){
  typedef std::vector<std::tuple<typename Anchored_Triangulation::Combinatorial_Map::Dart_const_handle,Point,Point,Point>> RealizationVector;
  RealizationVector realized_triangles;
  realized_triangles = triangulation.Base::lift();
...
void DemoWindowItem::draw_point(QPainter* painter, Point position){
  ...
  double point_y = -_poincare_disk_radius_in_pixels * CGAL::to_double(position.y());  // take the opposite so that the drawing is not upside down
...
DemoWindow::DemoWindow() : DemosMainWindow(){
  ...
  this->graphicsView->scale(0.5, 0.5); // rescale
...
// TODO: image is saved upside down. Fix it.
void DemoWindow::on_actionSave_as_SVG_triggered() {
    ...
    QRectF newSceneRect;
    QGraphicsScene *tempScene = new QGraphicsScene(_scene.sceneRect());

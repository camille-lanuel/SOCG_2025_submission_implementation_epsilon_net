...
template<class Traits>
	...
	public:
		...
		Hyperbolic_fundamental_domain_2<Traits> generate_small_systole_domain_g2(float p, int S);
  ...
  Complex_without_sqrt<float> random_complex_float_box(float p, int S);
...
template<class Traits>
Hyperbolic_fundamental_domain_2<Traits> Hyperbolic_fundamental_domain_factory_2<Traits>::generate_small_systole_domain_g2(float p, int S){
	...
	while (!is_domain_generated) {
		// 1. Generate inexact z0,z1,z2,z3
		Complex_without_sqrt<float> z0, z1, z2, z3;
		z1 = random_complex_float_box(p, S);
		z2 = random_complex_float_box(p, S);
		z3 = random_complex_float_box(p, S);
...
template<class Traits>
Complex_without_sqrt<float> Hyperbolic_fundamental_domain_factory_2<Traits>::random_complex_float_box(float p, int S)
{
	bool inverse = _random.uniform_int(0, 1);
	int s1 =  _random.uniform_int(1, S);
	int s2 = _random.uniform_int(1, S);
	// std::cout << "s1 " << s1 << " s2 " << s2 << std::endl;
	Isometry f1 = hyperbolic_translation<Traits>(_Point(p, 0), inverse);
	Isometry f2 = hyperbolic_translation<Traits>(_Point(0, p), true);
	// std::cout << "f2 " << f2.get_coefficient(0) << " ";
	// std::cout << f2.get_coefficient(1) << " ";
	// std::cout << f2.get_coefficient(2) << " ";
	// std::cout << f2.get_coefficient(3) << std::endl;
	_Point z = _Point(0, 0);
	for (int i = 0; i < s2; i++) {
		z = f2.evaluate(z);
		// std::cout << to_double(z.y()) << std::endl;
	}
	for (int i = 0; i < s1; i++) {
		z = f1.evaluate(z);
	}

	Complex_without_sqrt<float> res;
	float re = float(to_double(z.x()));
	float im = float(to_double(z.y()));
	res.set_real_part(re);
	res.set_imaginary_part(im);
  	// std::cout << "re = " << to_double(z.x()) << " im = " << to_double(z.y()) << std::endl;
	return Complex_without_sqrt<float>(re, im);
}
